...
@section('title', 'تعديل طريقة الشحن')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">تعديل طريقة الشحن</h1>
            ...
            <form action="{{ route('admin.shipping-methods.update', $shippingMethod) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $shippingMethod->code) }}" required>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="0" class="form-control @error('base_cost') is-invalid @enderror" id="base_cost" name="base_cost" value="{{ old('base_cost', $shippingMethod->base_cost) }}" required>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                                <option value="1" {{ old('is_active', $shippingMethod->is_active) ? 'selected' : '' }}>مفعل</option>
                                <option value="0" {{ old('is_active', $shippingMethod->is_active) ? '' : 'selected' }}>غير مفعل</option>
                            </select>
                            @error('is_active')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="weight_based" name="weight_based" value="1" {{ old('weight_based', $shippingMethod->weight_based) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="0" class="form-control @error('cost_per_kg') is-invalid @enderror" id="cost_per_kg" name="cost_per_kg" value="{{ old('cost_per_kg', $shippingMethod->cost_per_kg) }}">
                                <span class="input-group-text">ريال</span>
                            </div>
                            @error('cost_per_kg')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="0" class="form-control @error('free_shipping_threshold') is-invalid @enderror" id="free_shipping_threshold" name="free_shipping_threshold" value="{{ old('free_shipping_threshold', $shippingMethod->free_shipping_threshold) }}">
                                <span class="input-group-text">ريال</span>
                            </div>
                ...
                <div class="table-responsive mb-3">
                    <table class="table table-bordered" id="countriesTable">
                        ...
                        <tbody>
                            @foreach($shippingMethod->countries as $index => $country)
                                <tr id="country-row-{{ $index }}">
                                    <td>
                                        {{ $country->name }}
                                        <input type="hidden" name="countries[{{ $index }}][country_id]" value="{{ $country->id }}">
                                    </td>
                                    <td>
                                        <div class="input-group">
                                            <input type="number" step="0.01" min="0" class="form-control" name="countries[{{ $index }}][cost]" value="{{ $country->pivot->cost }}" required>
                                            <span class="input-group-text">ريال</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="countries[{{ $index }}][is_available]" value="1" {{ $country->pivot->is_available ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-danger remove-country" data-country-id="{{ $country->id }}" data-row-id="{{ $index }}">
...
<script>
    $(document).ready(function() {
        let countryRowCounter = {{ $shippingMethod->countries->count() }};
        ...
        $('#saveCountryBtn').click(function() {
            ...
            if (addedCountries.has(parseInt(countryId))) {
                alert('هذه الدولة مضافة بالفعل');
                return;
            }
            
            addedCountries.add(parseInt(countryId));
        ...
        $(document).on('click', '.remove-country', function() {
            ...
            addedCountries.delete(parseInt(countryId));

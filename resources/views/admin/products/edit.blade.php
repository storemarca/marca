...
@section('content')
            ...
            <div class="flex items-center">
                <div class="flex-shrink-0 mr-4">
                    @if($product->has_images)
                        <img src="{{ asset('storage/' . $product->main_image) }}" alt="{{ $product->name }}" class="h-16 w-16 rounded-lg object-cover border border-gray-200 shadow-sm">
                    @else
                        <div class="h-16 w-16 rounded-lg bg-blue-50 flex items-center justify-center text-blue-500 border border-blue-100 shadow-sm">
                ...
                <div>
                    ...
                    <div class="flex items-center mt-1 text-sm text-gray-600">
                        <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-full font-medium mr-2">{{ $product->sku }}</span>
                        <span class="text-gray-600 mr-2">|</span>
                        <span class="mr-1">الفئة:</span>
                        <span class="font-medium">{{ $product->category ? $product->category->name : 'غير مصنف' }}</span>
                    </div>
                </div>
            </div>
            <div class="mt-4 sm:mt-0">
                <span class="bg-{{ $product->status == 'active' ? 'green' : 'red' }}-100 text-{{ $product->status == 'active' ? 'green' : 'red' }}-800 text-sm font-medium px-3 py-1.5 rounded-full">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        ...
        <form action="{{ route('admin.products.update', $product->slug) }}" method="POST" enctype="multipart/form-data" id="product-form" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="cost" id="cost" value="{{ old('cost', $product->cost) }}" step="0.01" min="0" required
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors pr-8">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="weight" id="weight" value="{{ old('weight', $product->weight) }}" step="0.01" min="0" required
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors pr-12">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="4" required
                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- البلدان والأسعار -->
            <div class="p-5 bg-gray-50 rounded-lg border border-gray-200 mb-8">
                                        ...
                                        <label for="country_{{ $country->id }}" class="mr-2 block text-sm font-medium text-gray-700 cursor-pointer select-none">
                                            <div class="flex items-center">
                                                <img src="{{ asset('images/flags/' . strtolower($country->code) . '.svg') }}" 
                                                    ...
                                                    data-fallback="{{ asset('images/flags/placeholder.svg') }}"
                                ...
                                <div class="flex flex-col space-y-3">
                                    <div class="flex items-center">
                                        ...
                                        <input type="number" name="price[{{ $country->id }}]" 
                                            ...
                                            step="0.01" min="0" placeholder="0.00"
                                                class="border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 w-full pr-8">
                                    ...
                                    <div class="flex items-center">
                                        <label class="inline-flex items-center cursor-pointer select-none">
                                            <input type="hidden" name="is_active[{{ $country->id }}]" value="0">
                                            <input type="checkbox" name="is_active[{{ $country->id }}]" value="1" 
                                                {{ old('is_active.' . $country->id, $is_active[$country->id] ?? 0) ? 'checked' : '' }}
                                                class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded-sm">
            ...
            <div class="p-5 bg-gray-50 rounded-lg border border-gray-200 mb-8">
                ...
                <div class="mb-8">
                    ...
                    <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="flex">
                                                    <input type="color" name="colors[{{ $index }}][code]" value="{{ $color['code'] ?? '#000000' }}" 
                                                        class="h-10 w-10 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                                    <input type="text" name="colors[{{ $index }}][code_text]" value="{{ $color['code'] ?? '#000000' }}" 
                                                        class="mr-2 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                                </div>
                                            </div>
                        ...
                        <div class="mt-4">
                            <button type="button" id="add-color" class="bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 rounded-md transition-colors">
                                <svg class="w-5 h-5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="mb-8">
                    ...
                    <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                        ...
                        <div class="mt-4">
                            <button type="button" id="add-size" class="bg-amber-600 hover:bg-amber-700 text-white py-2 px-4 rounded-md transition-colors">
                                <svg class="w-5 h-5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                </svg>
                                إضافة مقاس جديد
                ...
                <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                @foreach($product->images as $index => $image)
                                    <div class="relative group">
                                        ...
                                        <div class="absolute inset-0 bg-indigo-900 bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center rounded-lg">
                    ...
                    <div>
                        ...
                        <div class="flex items-center justify-center w-full">
                            <label for="product_images" class="flex flex-col items-center justify-center w-full h-64 border-2 border-indigo-300 border-dashed rounded-lg cursor-pointer bg-white hover:bg-indigo-50 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <svg class="w-10 h-10 mb-3 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="p-5 bg-gray-50 rounded-lg border border-gray-200 mb-8">
                ...
                <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div class="mt-4">
                        <button type="button" id="add-video" class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-md transition-colors shadow-sm">
                            <svg class="w-5 h-5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                            </svg>
                            إضافة فيديو جديد
            ...
            <div class="sticky bottom-0 bg-white p-4 border-t border-gray-200 shadow-inner rounded-b-lg mt-8">
                <div class="flex items-center justify-end space-x-3 rtl:space-x-reverse">
                    <a href="{{ route('admin.products.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ml-2 transition-colors duration-200">
                    ...
                    <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200">
                    <svg class="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
